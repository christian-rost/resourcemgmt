"""Reports router: PDF-Export, Excel-Export."""
import io
import logging
import zipfile
...
from ..services.excel_service import generate_timesheet_excel
...
async def export_excel(
    ...
    """Excel export of monthly timesheet per customer/project using the XQT5 template."""
    role = current_user.get("role")
    effective_user_id = user_id if role in ("admin", "manager") and user_id else current_user["id"]
    ...
    excel_bytes = generate_timesheet_excel(
        entries=entries,
        project=project,
        year=year,
        month=month,
    ...
    month_str = f"{year}-{month:02d}"
    proj_name = (project.get("name") or "projekt").replace(" ", "_")
    filename = f"Leistungsnachweis_{proj_name}_{month_str}.xlsx"
...
async def export_excel_all(
    ...
    """Export Excel Leistungsnachweise for all users as ZIP."""
    start = date(year, month, 1).isoformat()
    end = date(year + 1, 1, 1).isoformat() if month == 12 else date(year, month + 1, 1).isoformat()
    ...
    with zipfile.ZipFile(zip_buf, "w", zipfile.ZIP_DEFLATED) as zf:
        for user in users:
            ...
            proj_name = (project.get("name") or "Projekt").replace(" ", "_")
            safe_name = (user.get("username", "user")).replace(" ", "_")
            zf.writestr(f"Leistungsnachweis_{proj_name}_{month_str}_{safe_name}.xlsx", excel_bytes)
            file_count += 1
    ...
    return StreamingResponse(
        zip_buf,
        media_type="application/zip",
        headers={"Content-Disposition": f"attachment; filename=Leistungsnachweis_{proj_name}_{month_str}_alle.zip"},
    )
